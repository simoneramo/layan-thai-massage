...
export default function ServicesPage() {
  return (
    <>
        ...
        <div className="mx-auto max-w-6xl px-5 py-16 sm:px-6 md:py-20 lg:px-8">
          <nav className="hero-anim text-sm text-plum-500" aria-label="Breadcrumb">
            <ol className="flex items-center gap-2">
              ...
              <li className="font-medium text-plum-700" aria-current="page">Treatments</li>
            </ol>
          </nav>
          <div className="mt-6 max-w-3xl">
            ...
            <div className="hero-anim d2 mt-8">
              <span className="block text-xs font-semibold uppercase tracking-[0.18em] text-plum-900">Massage types</span>
      ...
      {/* TRADITIONAL THAI */}
      <section id="traditional-thai" className="bg-plum-50 py-20 md:py-24">
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-5 sm:px-6 lg:grid-cols-2 lg:gap-14 lg:px-8">
          ...
          <div className="reveal order-1 lg:order-2">
            ...
            <h3 className="mt-6 font-heading text-sm font-bold uppercase tracking-wider text-plum-800">What to expect</h3>
            <ul className="mt-3 grid gap-2.5">
              {["Fully clothed, no oil", "Firm palm & thumb pressure", "Yoga-like assisted stretches", "Especially good for sport & work pain"].map((t) => (
                <li key={t} className="flex items-start gap-2.5 text-plum-700"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-500" /> {t}</li>
              ))}
            </ul>
          </div>
        </div>
      ...
      {/* ESSENTIAL OIL */}
      <section id="essential-oil" className="bg-white py-20 md:py-24">
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-5 sm:px-6 lg:grid-cols-2 lg:gap-14 lg:px-8">
          <div className="reveal">
            ...
            <h3 className="mt-6 font-heading text-sm font-bold uppercase tracking-wider text-plum-800">Lovely for</h3>
            <ul className="mt-3 grid gap-2.5">
      ...
      {/* FOOT & REFLEXOLOGY */}
      <section id="foot-reflexology" className="bg-plum-50 py-20 md:py-24">
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-5 sm:px-6 lg:grid-cols-2 lg:gap-14 lg:px-8">
          <div className="reveal img-frame order-2 aspect-[4/3] lg:order-1">
      ...
      {/* TARGETED */}
      <section id="targeted" className="bg-plum-900 py-20 md:py-24">
          ...
          <ol className="mt-14 grid gap-8 md:grid-cols-3">
            {[
              ...
              <li key={n} className="reveal flex gap-4">
                <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-plum-700 font-heading text-lg font-bold text-white">{n}</span>
                <div>
                  <h3 className="font-heading text-lg font-bold text-plum-800">{title}</h3>
        ...
        <div className="mx-auto max-w-4xl px-5 sm:px-6 lg:px-8">
          ...
          <div className="reveal mt-10 overflow-hidden rounded-2xl ring-1 ring-plum-100">
            <table className="w-full text-left">
              ...
              <thead>
                <tr className="bg-plum-50 text-plum-800">
              ...
              <tbody className="divide-y divide-plum-100 bg-white text-plum-700">
                {priceRows.map(([t, dur, price], i) => (
                  <tr key={i}>
                    <td className="px-5 py-3.5 font-medium text-plum-900">{t}</td>
                    <td className="px-5 py-3.5 text-right">{dur}</td>
                    <td className="px-5 py-3.5 text-right font-semibold text-plum-700">{price}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="reveal mt-4 text-sm text-plum-900">Prices are per session. Please call to book — walk-ins are welcome when space allows.</p>
        </div>
      </section>
